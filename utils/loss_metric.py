...
def get_occupancy_depth_score(inputs, outputs, threshold_list, xy_range = 52):

    # pdb.set_trace()
    empty_probs = torch.zeros_like(inputs['total_depth_empty'][..., 0])
    # empty_probs = torch.zeros(inputs['total_depth_empty'][:2])

    mask = inputs['mask']
    empty_probs[~mask] = outputs['depth_empty'].to('cpu')

    empty_probs = empty_probs.numpy()
    first_dimension = np.arange(0, empty_probs.shape[0])
    ...
    point_error = {}
    gt_position = inputs['surface_point']
    ...
    if xy_range == 52:
        ...
        closer_mask = get_closer_point_mask(gt_position, xy_range=xy_range)
        gt_position = gt_position[~closer_mask]
        origin = inputs['origin'][~closer_mask]


    for idx, threshold in enumerate (threshold_list):

        empty_probs[:, -1] = threshold
        empty_binary = (empty_probs >= threshold)

        first_position = empty_binary.argmax(axis=1)


        # get the first occupancy position
        total_position = inputs['total_depth_empty']
        pred_position = total_position[first_dimension, first_position]

        if xy_range == 52:
            pass
        else:
            pred_position = pred_position[~closer_mask]

        gt_depth = (origin - gt_position).norm(dim=1).numpy()
        pred_depth = (origin - pred_position).norm(dim=1).numpy()


        if threshold == 0.2:
            point_error['gt_position'] = gt_position
            point_error['pred_position'] = pred_position
            point_error['origin'] = inputs['origin']

        abs_rel, sq_rel, rmse, rmse_log, a1, a2, a3 = compute_errors(gt_depth, pred_depth)
        total_abs_rel.append(abs_rel)
        total_sq_rel.append(sq_rel)
        total_rmse.append(rmse)
        total_rmse_log.append(rmse_log)
        total_a1.append(a1)
        total_a2.append(a2)
        total_a3.append(a3)

    return total_abs_rel, total_sq_rel, total_rmse, total_rmse_log, total_a1, total_a2, total_a3, point_error
