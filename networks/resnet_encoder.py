...
class ResnetEncoder(nn.Module):
    ...
    def freeze_cams(self, freeze=True):
        ...
        self.density = torch.nn.Parameter(torch.zeros([1, 1, 1])) # 分布式没有可训练参数会报错，初始化一个可训练参数
...
class Encoder_res101(nn.Module):
    def __init__(self, C = 64, path = None, network_type = '101'):
        ...
        self.Upsampling_4 = Upsampling_4(512, 256)  # 自己增加的，使得feature 1/4 image
    ...
    def forward(self, x):
        ...
        # print('x1 :', x.min(), x.max())
        #
        # x = (x - 0.45) / 0.225
        #
        # print('x2:', x.min(), x.max())

        # eps_time = time.time()

        # pdb.set_trace()

        # x = x - 0.5 # # go to -0.5, 0.5
