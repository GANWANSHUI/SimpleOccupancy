...
class DepthGenerator(object):
    ...
    def __call__(self, num_workers=1):
        ...
        def process_one_sample(index):
            ...
            for cam in self.camera_names:
                ...
                car_egopose_to_sensor = np.vstack([
                    np.hstack((sensor_rotation.rotation_matrix, sensor_translation)), np.array([0, 0, 0, 1])])
                ...
                valid_mask = (pixel_uv[:, 0] >= 0) & (pixel_uv[:, 0] <= width - 1) & (pixel_uv[:, 1] >= 0) & (pixel_uv[:, 1] <= height - 1)
...
# python export_gt_depth_nusc.py train_full